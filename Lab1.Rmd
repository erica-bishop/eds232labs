...
This visualization isn't especially useful because it doesn't show any clear trend in price over the season, which is surprising. I think this is because the plot doesn't group by region or type, which are two variables that likely have a large influence on price.
...
There are 10 cities represented (each represented with an integer 0-9).
...
Variety is the most highly correlated variable with price. This makes sense because different types of pumpkins are likely to have different resource requirements and therefore different prices, whereas the city and package type with have a comparatively smaller impact on price. The correlation coefficient shows that pumpkin variety accounts for about 86% of the variation in price.
...
The usefulness of a linear model is contingent upon four factors: exogeneity of the dependent and independent variables, variation in the independent variable, normal distribution of errors, and linear parameters. With this data set, price and package are not exogenous (there are other factors that influence price, such as variety, as we already explored with the correlation coefficients). It's also not clear that there is a linear relationship between price and package. Therefore, a linear regression model is probably not the best interpretation of the data in this case.
