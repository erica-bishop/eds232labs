...
```{r recipe}
#preprocessing
knn_recipe <- recipe(Attrition ~., data = churn_train) |> 
  step_dummy(all_nominal(), -all_outcomes(), one_hot = TRUE) |> #encode nominal variables - be careful though that all_nominal will include outcome variable
  step_normalize(all_numeric(), -all_outcomes()) |>  #normalize distances
  prep() #applies pre-processing steps to training data and saves as recipe

#bake 
  baked_churn <- bake(knn_recipe, churn_train)
...
#bake test data too - but SEPARATELY!
baked_test <- bake(knn_recipe, churn_test)
...
knn_spec <- nearest_neighbor() |> 
  ...
  set_mode("classification") #specify regression or classificaton
...
knn_fit
#output shows best k (hyperparameter - 5 is the default (the only one it tested))
```


```{r cv}
#do cross-validation to tune the parameter

set.seed(123)
# 10-fold CV on the training dataset
cv_folds <- churn_train |> 
  vfold_cv(v = 5) #10 is default but doing 5 here
...
wf_knn_tune <- workflow() |> 
  ...
  add_recipe(knn_recipe) #same recipe as before because only change happened to specification
...
#tuning grid is a grid of hyperparameters to be tuned - in this case just k (1 column)
#for each value of k in the grid, model will try on every fold - this can make computation get crazyyy
#list of neighbors specifies try k as 1, 5, and up to 100 by 10s
...
final_fit <- final_wf |> 
  last_fit(churn_split) #using churn split fits onto training and makes predictions on test
